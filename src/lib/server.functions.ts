...
export const getOdinFullData = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    try {
      const queries = [
        "SELECT id, username, password, IFNULL(exp_date, 0), enabled, admin_enabled, is_trial, is_restreamer, is_isplock, max_connections, bouquet, admin_notes, reseller_notes, allowed_ips, allowed_ua, forced_country, created_by FROM users ORDER BY id DESC",
