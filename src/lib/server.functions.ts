...
export const getUsers = createServerFn({ method: "GET" })
  .handler(async () => {
    ...
    try {
      ...
      const query = "mysql -u user_iptvpro -p'Y92RYuXHLP58AbOciQW' -h 23.158.72.30 -P 7999 -e \"SELECT id, username, password, exp_date, admin_enabled, enabled, (SELECT count(*) FROM xtream_iptvpro.user_activity_now WHERE user_id = u.id) as active_cons, max_connections FROM xtream_iptvpro.users u LIMIT 100;\" --batch --json";
